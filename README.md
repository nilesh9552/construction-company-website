...
4. Set Cloudinary environment variables instead of a local upload directory:
   - `CLOUDINARY_CLOUD_NAME`
   - `CLOUDINARY_API_KEY`
   - `CLOUDINARY_API_SECRET`
   - `CLOUDINARY_FOLDER` (optional, default `construction_website`)
5. Build the container with `docker build -t construction-company-website .`.
6. Run it with port `8080` exposed.
...
6. If you add a Render PostgreSQL database later, set the datasource and JPA env vars too.
